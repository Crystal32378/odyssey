...
export async function POST(request: Request) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: process.env.HOMER_MODEL || "gpt-5.6-sol", reasoning: { effort: "low" }, input: [{ role: "system", content: `${constitution}\n${instruction}` }, { role: "user", content: JSON.stringify(payload) }], text: { format: { type: "json_schema", name, strict: true, schema } }, max_output_tokens: phase === "summary" || phase === "card" ? 900 : 500 }) });
...
function invalidInput(field: string, status = 400) { return NextResponse.json({ error: "HOMER_INPUT_INVALID", message: `Invalid ${field}.` }, { status }); }
