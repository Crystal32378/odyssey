  ...
  return <main className={`journey${lunaTriggerId && !lunaSeen && !lunaPresentationDone ? " journey-luna-active" : ""}`}>
    ...
    <section className={`island-layout island-${island.id} content-${presentation.contentSide} scrim-${presentation.scrimDirection}`} key={island.id} style={presentationStyle}>
      ...
      <article className={`narrative${lunaTriggerId && !lunaSeen && !lunaPresentationDone ? " narrative-has-luna" : ""}`}>
        ...
        {lunaSettled && <section className="choice-beat stage-step stage-step-question"><p className="choice-label">THE SHORE ASKS</p><p className="island-question">{scene.question}</p></section>}
        {lunaSettled && <div className="response-band stage-step stage-step-response">
          ...
          <div className="journey-actions"><button className="answer-shore" onClick={resolveAnswer} disabled={!answer.trim()}>{island.id === "ithaca" ? "COMPLETE THE RETURN" : "CONTINUE THE JOURNEY"}</button>{island.id !== "ithaca" && <small className="answer-memory-note">Your words will be remembered.</small>}{errorMessage && <ErrorBox message={errorMessage} requestId={requestId} retry={resolveAnswer}/>}</div>
          <div className="journey-system-actions"><button className="new-voyage" onClick={resetJourney}>Restart journey</button></div>
        </div>}
      </article>
    </section>
  </main>;
...
function Map({ goal, setGoal, begin, error }: { goal: string; setGoal: (v: string) => void; begin: () => void; error: string }) { return <main className="landing map-world"><link rel="preload" as="image" href="/assets/ship-token.webp"/><div className="map-plane"><img className="world-map" src="/odyssey-map.png" alt="The fourteen shores of the Aegean world"/><div className="island-layer" aria-hidden="true">{ISLANDS.map((island, i) => <button type="button" key={island.id} className={`island-node node-${i + 1}${i === 0 ? " is-start" : ""}`}><b>{island.name.toUpperCase()}</b></button>)}</div></div><div className="map-wash"/><header className="world-title"><span>ODYSSEY</span><i>✦　返 鄉 之 旅　✦</i></header><section className="world-entry"><h1>SPEAK YOUR ITHACA</h1><p className="world-prompt">What are you trying to return to—<br/>a person, a place, a promise, or a part of yourself?</p><div className="world-form"><label htmlFor="ithaca-goal">YOUR ITHACA</label><input id="ithaca-goal" aria-label="Your Ithaca" value={goal} onChange={(e) => setGoal(e.target.value)} onKeyDown={(e) => e.key === "Enter" && begin()} placeholder="For example: my family, my courage, a life that feels like mine…"/><button onClick={begin}>BEGIN THE JOURNEY</button></div><p className="world-stakes">Your words will travel with you—and return to you at the end.</p>{error && <p className="map-error" role="alert">{error} <button onClick={begin}>TRY AGAIN</button></p>}</section><footer className="world-mark"><span>14 ISLANDS</span><span>✦</span><span>ONE JOURNEY</span><span>✦</span><span>COUNTLESS TRUTHS</span></footer></main>; }
